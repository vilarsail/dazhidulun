...
def parse_md_blocks(md_content):
    ...
    for b in blocks:
        ...
        b['clean_text'] = clean_chinese(b['original_text'])
...
def process_file(file_idx):
    ...
    md_search_start = 0
    # Match line by line directly against the block's clean text
    # This avoids the SequenceMatcher autojunk issue on huge strings
    for t_idx, txt_line in enumerate(txt_lines):
        ...
        for m_idx in range(md_search_start, min(md_search_start + 50, len(md_blocks))):
            ...
            # Fallback to SequenceMatcher WITHOUT autojunk
            s = difflib.SequenceMatcher(None, clean_t, md_clean, autojunk=False)
            sim = s.ratio()

            # Since md_clean can be long, let's also check substring similarity
            c_len = len(clean_t)
            if len(md_clean) > c_len:
                for i in range(len(md_clean) - c_len + 1):
                    sub_s = difflib.SequenceMatcher(None, clean_t, md_clean[i:i+c_len], autojunk=False)
                    sub_sim = sub_s.ratio()
                    if sub_sim > sim:
                        sim = sub_sim
                        if sim > 0.95: break
    ...
    # Sort reverse to insert without corrupting indices
